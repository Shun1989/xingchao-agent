...
- **Main process** `electron/main.ts`: the assembly root. It creates
  ...
  `app.getPath("userData")` — `SettingsStore` / `AuthStore` / `AttentionStore`
...
The Agent workspace mounts the four typed Link tools for either Link backend and seven integrated
browser tools for every runtime. OOMOL adds team
identity (`--team`) and the bundled oo Skills; OpenConnector adds neither. OOMOL direct `oo`
...
| `config.ts`           | `buildOpencodeConfig()`: provider `oomol` (default Auto/`oopilot`, npm `@ai-sdk/openai-compatible`, baseURL=`llmBaseUrl`, also carrying the other OpenAI-compatible built-in models) + provider `openai` (GPT 5.6 Sol/Terra/Luna, OpenAI Responses runtime). Auto, GPT, and Qwen use a 256K input pricing-tier guard; OpenCode's 20K reserve therefore triggers compaction at 236K while each model retains its declared physical context and output limits. DeepSeek V4 alone omits that guard: its 1M physical context and 32K output budget trigger compaction at 968K in normal sessions. Note: the gateway `/v1/models` **does not list** `oopilot` — it is a gateway-side alias routed to the real model by chat/completions, so do not "correct" the Auto model name from the models list. It overrides OpenCode's native `build` / `plan` agents: Build uses `WANTA_SYSTEM_PROMPT`, Plan uses `WANTA_PLAN_SYSTEM_PROMPT`. `external_directory`, `edit`, and any local `bash` other than a direct `oo` / `$WANTA_OO_BIN` / `${WANTA_OO_BIN}` all go through ask first, then the ChatService main-process local-access policy (Default Access / Full Access; Default Access auto-approves ordinary bash, scripts, project checks, data processing, simple output filtering, and non-sensitive reads including broad home/system discovery, pausing only at consequential boundaries — credentials/private application data, broad edits, destructive deletes, global/system or alternate-source dependency changes, privilege escalation, pushes, publish/deploy, and infrastructure changes. Direct Python requirements are auto-approved through an exact turn-private `.wanta-python` or selected-project `.venv` / `venv` interpreter, directly or through `uv pip --python`; direct Node.js packages are auto-approved only when npm/pnpm/yarn/bun explicitly targets the turn process directory or selected project. Both require no explicit source override, while package name, size, runtime, package runners, and unfamiliar ordinary flags are not confirmation boundaries. Full Access = session-level local YOLO after OpenConnector credential and injected-runtime hard-deny checks). A direct `oo` / `$WANTA_OO_BIN` / `${WANTA_OO_BIN}` command keeps OpenCode's fast allow only for the OOMOL Link runtime. OpenConnector retains `bash: "ask"` so ChatService can apply its runtime-specific hard-deny and local-access policy. Plan's `edit` is limited to `.opencode/plans/*.md`. The root-level `WANTA_PERMISSION` mirrors the Build permission; no tools disable table is emitted (all built-in tools stay enabled). Each custom OpenAI-compatible model additionally gets its own provider `wanta-custom-<id>` (npm `@ai-sdk/openai-compatible`, the user's own baseURL/apiKey, env-only). |
| `system-prompt.ts`    | `WANTA_SYSTEM_PROMPT` (English): a Worker / task-first stance — first decide the work result the user wants, then pick the shortest reliable path among a direct answer, Local tools, and Link tools; use Local tools when local context is needed (bash, files, scripts, specific URLs; cwd is a private scratch, use absolute paths or `~` for real files); use the managed `wg` command from bash when a pinned WikiGraph archive is relevant; use `list_apps` for the current workspace's connection list, but never as a health check before an ordinary SaaS action; go to Link tools (the search→inspect→call flow) only when the task genuinely needs a connected account / SaaS data or action. A bare `oo` CLI inside a provider skill is preferred as a capability reference; when a Link tool cannot do it and the task genuinely needs it, it may still run, but must use the current turn's selector. It keeps the inspect-before-call, authorization-required blocking, minimal-access/payload, and side-effect-confirmation contracts, plus an "Asking the user" structured-question contract (header spec, no re-asking after a rejection, etc.).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `tool-sources.ts`     | The **String.raw inline source** of the four Link custom tools (`list_apps.ts` / `search_actions.ts` / `inspect_action.ts` / `call_action.ts`) and seven integrated-browser tools, exported as `AGENT_TOOL_FILES` and `BROWSER_AGENT_TOOL_FILES`. The tools import the build-time-merged tool helper + Zod from the workspace-private `../runtime/tool.js`, so tool loading does not depend on OpenCode implicitly installing npm packages on the user's machine. They run in OpenCode's Bun and take no part in this project's tsc/oxlint. The four Link tools call `oo` via `execFile` (path from `process.env.WANTA_OO_BIN`, falling back to `"oo"` — note that a packaged / Finder-launched GUI process **does not inherit the shell PATH** (PATH is empty), so binaries always use an absolute path (`WANTA_OO_BIN` env / `resolveBundledBin`) and the `"oo"` string fallback is a dev-only crutch, unusable in production). WikiGraph is deliberately outside this tool layer: `manager.ts` creates a Wanta-owned `wg` shim under `<userData>/agent/bin` and prepends that directory to the sidecar `PATH`; `electron/knowledge/wg.ts` calls WikiGraph's SDK runner with Wanta's private state dir and forwards terminal streams/exit code without wrapping business output. Only `list_apps` and `call_action` pass `--team` on the main `oo` call; `search_actions`' own `oo connector search` carries no workspace selector (catalog search is identity-independent) — the team identity applies only to its internal `connector apps` list call (which overrides the active-workspace `authenticated` field) and its `/v1/providers` request. `call_action` translates a stderr `errorCode: <code>` that hits the AUTH_BLOCKING set into `{status:"authorization_required", authUrl: <console>/app-connections?provider=...}` (the authUrl base is taken only from the `WANTA_CONSOLE_URL` env; when missing it returns a structured `config_missing` error rather than hardcoding an endpoint).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `workspace.ts`        | `ensureAgentWorkspace(rootDir, bundledSkillsDir?, bundledToolRuntimePath?)`: on every startup it idempotently overwrites `<userData>/agent/workspace/.opencode/tools/*.ts` (using `node:fs/promises`; the directory name is the **plural `tools`**, verified against 1.18.10 — upstream docs disagree, do not change to singular), syncs the build-time-merged `.opencode/runtime/tool.js`, and rebuilds `.opencode/skill/<name>/` from the bundled built-in skills as source of truth (source = `resources/skills`; `scripts/skills.ts` combines the four oo-exported Skills with tracked Wanta-owned Skills). Browser is runtime-neutral and is synchronized for every runtime, while the OOMOL-specific bundle is included only for OOMOL. OpenCode scans the cwd's `.opencode/{skill,skills}/**/SKILL.md`, so Wanta's own agent reads these Skills directly — it **no longer releases skills into other AI agents' home dirs**. `.opencode/skills/` is the private target dir for Wanta registry/runtime skills, written by `SkillServiceImpl` and not wiped by the bundled-skill sync; the Wanta registry cache and external agent skills (Claude Code / Codex / Universal, etc.) are all synced here first, then handed to the sidecar to load. The workspace is the sidecar's cwd and is **immutable** (the custom tools depend on the `.opencode/tools/` beneath it).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `event-translator.ts` | Statelessly translates OpenCode SSE (`message.updated` / `message.part.updated` / `session.compacted` / `session.error` / `permission.asked` / `permission.v2.asked` / `question.asked` / `question.v2.asked` / `question.replied` / `question.rejected`, etc.) → ChatService ServerEvents (0..n emits per event), maps compaction markers to user-facing lifecycle activity, and filters internal checkpoint summaries and synthetic continuation messages from persisted history; `parseAuthorization()` recognizes the call_action authorization-signal JSON; permission events first enter the ChatService main-process local-access policy — auto-approvable requests continue straight through the OpenCode permission reply API, and only requests still needing a human decision are mapped to an in-chat permission state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
`--team` arguments apply only to the OOMOL Link runtime. OpenConnector uses endpoint-only
...
`electron/browser/` owns the visible browser surface. The unregistered `BrowserManager` keeps at
most three task-scoped `WebContentsView` pages in an account-scoped persistent Electron partition;
the registered `BrowserServiceImpl` is the renderer facade for toolbar/bounds and user-initiated
browser settings actions. Page contents have no Wanta preload or Node integration.

Stable `playwright-core` connects to one view through a minimal single-target CDP relay backed by
`webContents.debugger`. The agent's seven flat browser tools call an authenticated loopback server
because the OpenCode sidecar cannot import Electron. The tool runtime supplies its OpenCode session
ID outside the model-visible action arguments, and the main process uses that ID as the task page
key. Browser settings can disable the capability, clear the current account profile, and open the
system Downloads folder; page downloads are owned by that profile and always use the system
Downloads directory. See [integrated-browser.md](integrated-browser.md) for the product,
access-mode, lifecycle, and Chrome login-state decisions.
...
electron/
  ...
  browser/ common,node,page,playwright-relay,control-server,policy(+test)  visible task browser, Playwright CDP relay, loopback agent bridge
