...
- **Core data flow**: user message → OpenCode Agent → the selected Link runtime (OOMOL Connector or
  a user-configured OpenConnector, both through the bundled `oo` CLI) / local tools → streamed reply.
...
| **OpenConnector** | Open-source sibling in the same connector ecosystem ([github.com/oomol-lab/open-connector](https://github.com/oomol-lab/open-connector)). Users may select an existing OpenConnector deployment as Wanta's Link runtime by configuring separate API and Console origins plus an optional runtime token. OpenConnector supplies connector actions, not an LLM, and Wanta does not launch, bundle, migrate, or administer it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **WikiGraph**     | The parsing and retrieval runtime for local `.wikg` knowledge bases. `wiki-graph@0.4.0` and `wiki-graph-core@0.4.0` ship as exact-version npm dependencies; the main process uses the SDK directly, and the Agent sees a Wanta-owned `wg` command injected at the front of the OpenCode sidecar `PATH`. That command is built as `dist-electron/wanta-wg.js`, runs through Electron's Node mode, binds Wanta's private `userData/wikigraph-state`, and passes terminal streams/exit code through without wrapping WikiGraph business output. Knowledge-base imports are copied into WikiGraph's managed default library. Currently a Beta feature, off by default — the knowledge-base menu appears and sessions may reference knowledge bases only after the user enables it in Settings; the toggle persists in the local `settings.json`. Sessions persist only knowledge-base IDs and never send the raw archive as a chat attachment.                                                                                               |
...
| **OpenCode**      | The Agent kernel. Spawns the published binary `opencode-ai@1.18.10` as a sidecar; the main process drives it over HTTP+SSE via `@opencode-ai/sdk@1.18.10`. Pure configuration-level customization, zero source modification. At research time (2026-05) no community precedent was found for using OpenCode in non-IDE / general-agent workloads; Wanta was the first known case (positioned as a non-coding agent at inception; local coding capability was opened up later).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Strictly non-coding agent: deny all built-in coding tools, keep only connector tools                   | **Opened up but controlled**: OpenCode built-in tools are enabled; under Default Access, bash, ordinary file read/write, specific non-sensitive paths, direct Python and Node.js dependencies in bounded task/selected-project environments, package runners, browser tooling, and unfamiliar ordinary flags flow without package-name or parser allowlists. Baseline safety boundaries — credential/secret paths, destructive deletion, global/system or explicit alternate-source dependency changes, privilege escalation, pushing, publishing/deploying, etc. — require approval via an in-chat confirmation card |
| Custom tools were only `search_actions` / `call_action`                                                | Grew to four connector tools plus seven visible integrated-browser tools. Connector additions are `inspect_action` (enforcing inspect-before-call) and `list_apps` (directly lists connected apps in the active workspace). WikiGraph access uses Wanta's managed `wg` command on the sidecar `PATH`, not a custom OpenCode tool.                                                                                                                                                                                                                                                                                     |
