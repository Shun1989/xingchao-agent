...
test("local runtime config omits Connector guidance and oo command permission shortcuts", () => {
  ...
  assert.match(WANTA_LOCAL_SYSTEM_PROMPT, /load and follow the `wikigraph-knowledge` Skill/)
  assert.match(WANTA_LOCAL_SYSTEM_PROMPT, /selected library\/archive URI from the turn context/)
  assert.doesNotMatch(WANTA_LOCAL_SYSTEM_PROMPT, /managed `wg` command/)
  assert.doesNotMatch(WANTA_LOCAL_SYSTEM_PROMPT, /do not rely on a global WikiGraph install/)
...
test("system prompt treats Link as a contextual capability, not the default path", () => {
  ...
  assert.match(WANTA_SYSTEM_PROMPT, /never edit, rename, move, or delete an attachment/)
  assert.match(WANTA_SYSTEM_PROMPT, /In Build mode.*copy the attachment into the current artifact directory/s)
  assert.match(WANTA_SYSTEM_PROMPT, /In Plan mode, do not copy or edit files/)
  ...
  assert.match(WANTA_SYSTEM_PROMPT, /Load and follow the `wikigraph-knowledge` Skill/)
  ...
  assert.doesNotMatch(WANTA_SYSTEM_PROMPT, /Wanta provides its own `wg` on PATH/)
  assert.doesNotMatch(WANTA_SYSTEM_PROMPT, /Search entity and triple scopes/)
  assert.doesNotMatch(WANTA_SYSTEM_PROMPT, /Evidence counts are supporting passage counts, not confidence/)
  assert.doesNotMatch(WANTA_SYSTEM_PROMPT, /Never modify a knowledge base unless the user explicitly asks/)
})
...
test("buildAgentLinkEnv keeps OpenConnector credentials out of OOMOL variables", () => {
  ...
  assert.equal(env.WANTA_CONNECTOR_URL, "http://127.0.0.1:3000")
  assert.equal(env.WANTA_CONSOLE_URL, "http://127.0.0.1:5173")
  assert.equal(env.WANTA_LINK_RUNTIME, "openconnector")
  assert.equal(env.WANTA_TEAM_NAME, undefined)
  assert.equal(env.WANTA_TEAM_SCOPE_PATH, "/tmp/scope.json")
})
